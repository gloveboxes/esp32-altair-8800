...
int max;
{
    ...
    while (1)
    {
        /* Use x_conin (BDOS 1) so the emulator can yield to other tasks
           while waiting for a key. BDOS 1 already echoes printable chars,
           so we must NOT echo them again (doing so caused doubled chars
           with the previous version). We only emit our own output for
           CR (add LF) and BS (finish the erase BDOS already started).
           A prior version polled x_keyrd in a tight loop -- that ran
           the 8080 hot and starved the rest of the system, making input
           feel laggy. */
        ch = x_conin() & 0x7F;
        ...
        if (ch == 8 || ch == 127)
        {
            ...
            {
                pos--;
                /* BDOS 1 already echoed the BS; finish the erase. */
                x_cout(' ');
                x_cout(8);
            }
            else
            {
                /* Undo the BS BDOS echoed when there is nothing to erase. */
                x_cout(' ');
            }
            continue;
        }
        ...
        {
            buf[pos++] = ch;            /* do not echo: BDOS 1 already did */
        }
    }
    buf[pos] = 0;
    return pos;
